...
init = np.array([r, c]).T
a= 0.005 #a代表了tangent
b = 0.1 #beta代表了courbure b越小说明courbure可以越大，因为我们实际上最终是要minimiser总的能量
#w_l = 1
w_e = 1 #用于处理image
g = 0.0001 #stepwise, 是inertie
bc = 'fixed'
...
ax.set_xticks([]), ax.set_yticks([])
ax.axis([0, im.shape[1], im.shape[0], 0])
title = 'retine/init'+str(init_x)+'_'+str(init_y)+'alpha'+str(a)+'beta'+str(b)+'w_line'+str(w_l)+'w_edge'+str(w_e)+'gamma'+str(g)+'.png'
plt.savefig(str(title))
plt.show()
